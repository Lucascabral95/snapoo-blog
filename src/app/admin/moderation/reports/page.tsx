...
export default async function ModerationReportsPage() {
  ...
  return (
    <main style={{ maxWidth: 1100, margin: "40px auto", padding: "0 24px" }}>
      ...
      {reports.length === 0 ? <p>No hay denuncias pendientes.</p> : (
        <ul>
          {reports.map((report) => (
            <li key={String(report._id)} style={{ margin: "16px 0" }}>
              <strong>{report.priority === "critical" ? "CRÍTICO · " : ""}{report.reason}</strong>{" "}
              <span>{report.targetType} · {report.status}</span>{" "}
              <small>{new Date(report.createdAt).toLocaleString("es-AR")}</small>
            </li>
          ))}
        </ul>
      )}
